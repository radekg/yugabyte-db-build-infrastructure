...
cd /yb-source
./yb_build.sh debug --gcc --sj # don't build java here, we might not need it

case "${1}" in
    cpp)
        ./yb_build.sh debug --ctest --gcc
        ;;
    cxx)
        if [ "$#" -eq 1 ]; then
            usage-cxx
            ...
            if [ "$#" -eq 3 ]; then
                ./yb_build.sh debug --gcc --cxx-test "${2}" --gtest_filter "${3}"
            else
                ./yb_build.sh debug --gcc --cxx-test "${2}"
            fi
        fi
        ;;
    java)
        if [ "$#" -eq 1 ]; then
            shift
            ./yb_build.sh debug "$@" --gcc --java-tests
        else
            testpath="${2}"
            shift
            shift
            ./yb_build.sh debug "$@" --gcc --java-test "${testpath}"
        fi
        ;;
    raw)
        shift
        ./yb_build.sh debug "$@" --gcc
        ;;
    *)
        usage
        ;;
